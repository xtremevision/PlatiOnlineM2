...
//                new \Magento\Framework\Phrase('The padCrypt class must be loaded for AES_Encryption to work: '.$padCrypt_url)
//            );
//        }

        if (!function_exists('mcrypt_module_open')) {
            throw new LocalizedException(
                new \Magento\Framework\Phrase('The mcrypt extension must be loaded.')
            );
        }

        if (strlen($initVector) != 16 && in_array($mode, $this->vector_modes)) {
            throw new LocalizedException(
                new \Magento\Framework\Phrase('The $initVector is supposed to be 16 bytes in for CBC, CFB, NOFB, and OFB modes.')
            );
        } elseif (!in_array($mode, $this->vector_modes) && !empty($initVector)) {
            throw new LocalizedException(
                new \Magento\Framework\Phrase('The specified encryption mode does not use an initialization vector. You should pass an empty string, zero, FALSE, or NULL.')
            );
        }
        ...
        if (!in_array($this->encryption, $this->allowed_bits)) {
            throw new LocalizedException(
                new \Magento\Framework\Phrase('The $key must be either 16, 24, or 32 bytes in length for 128, 192, and 256 bit encryption respectively.')
            );
        }
        ...
        if (!in_array($mode, $this->allowed_modes)) {
            throw new LocalizedException(
                new \Magento\Framework\Phrase('The $mode must be one of the following: '.implode(', ', $this->allowed_modes))
            );
        }

        if (!array_key_exists($padding, $this->allowed_paddings)) {
            throw new LocalizedException(
                new \Magento\Framework\Phrase('The $padding must be one of the following: '.implode(', ', $this->allowed_paddings))
            );
        }
    ...
    {
        return call_user_func_array(['\Xtreme\PlatiOnline\Helper\Crypt\PadCrypt', 'pad_'.$this->allowed_paddings[$this->padding]], [$text, $block_size]);
    }

    private function unpad($text)
    {
        return call_user_func_array(['\Xtreme\PlatiOnline\Helper\Crypt\PadCrypt', 'unpad_'.$this->allowed_paddings[$this->padding]], [$text]);
    }
